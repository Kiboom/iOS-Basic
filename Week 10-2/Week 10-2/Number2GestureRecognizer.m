...
#import "Number2GestureRecognizer.h"

@implementation Number2GestureRecognizer
...
- (BOOL)isFailedStep1GestureFrom:(CGPoint)prevTouch To:(CGPoint)curTouch {
    ...
    else if(_hasStep1HappendOnce && (curTouch.x - prevTouch.x >= 0) && (curTouch.y - prevTouch.y >= 0)){
        _isStep1 = NO;
        _isStep2 = YES;
        return NO;
    }
...
- (BOOL)isFailedStep3GestureFrom:(CGPoint)prevTouch To:(CGPoint)curTouch {
    ...
    else if(_hasStep3HappendOnce && (curTouch.x - prevTouch.x >= 0)){
        _isStep3 = NO;
        _isStep4 = YES;
        return NO;
    }
    else {
        // step4로 넘어가는 변곡점에 대한 기준 완화 : 너무 뾰족한 부분이라 적당히 얼렁뚱땅 써도 Step3를 만족시킨다고 판정.
...
- (BOOL)isFailedStep4GestureFrom:(CGPoint)prevTouch To:(CGPoint)curTouch {
    if((curTouch.x - prevTouch.x >= 0) && (curTouch.y - prevTouch.y <= 20)) {
...
- (NSNumber *)recognizeNumber {
    if(_isError==NO && _hasStep1HappendOnce==YES && _hasStep2HappendOnce==YES && _hasStep3HappendOnce==YES && _hasStep4HappendOnce==YES) {
        return @2;
    }
